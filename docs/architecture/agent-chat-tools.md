...
| **multi_evidence_speed_accuracy** | **Нет `final_answer` в `tool_trace`** | Цепочка `workspace_inspect` → `idea_search` → `paper_profile` → `paper_quote_search` → `paper_profile`; длинный текст ответа, предупреждения `no_quote_found`, `no_citations`. Похоже на исчерпание шагов без вызова финального тула. | Усилить в системном промпте / budget-ноде обязательность **`final_answer` после N тулов**; при «тонких» цитатах не тратить шаги на второй `paper_profile`. Рассмотреть повышение `agent_max_tool_calls` для многочастных вопросов. В Phoenix не смешивать выводы разных run: в одном `trace_id` возможны чужие спаны того же процесса — ориентир по цепочке `tool.*` вокруг `agent.query`. |

**Флаги скрипта:** `--suite default|heavy`, `--trace-audit`, `--dry-run`, `--write-report PATH` (append JSONL summary per run для CI). **Коды выхода:** `0` все кейсы ок (в т.ч. последний инструмент в `tool_trace` = `final_answer`, длина ответа ≥ 40), `1` провал приёмки, `2` не найден воркспейс, `3` `/health` не 200. С `--trace-audit` в эвристики входят streak нулевых `edge_search`, streak `paper_profile` с одним и тем же `work_id`, и счётчик шагов `cypher_query` с непустым `error` (см. [`science_graphrag/agent/agent_trace_audit.py`](../../science_graphrag/agent/agent_trace_audit.py)). Канон приёмки trace-audit: [`chat-agent-roadmap-trace-audit-2026-04-27.md`](../analysis/chat-agent-roadmap-trace-audit-2026-04-27.md). Общие практики по промптам и аудиту бандла: [`agent-tools-best-practices.md`](agent-tools-best-practices.md). Развёрнутый бэклог по фазам (A–D): [`docs/analysis/agent-chat-tools-work-plan-2026-04-28.md`](../analysis/agent-chat-tools-work-plan-2026-04-28.md).
