...
| Agent v3 quality judge (`agent_v3_quality_judge_v1`) | Advisory | Wave B: реализация lane; **Wave C:** rollout/KPI/baseline/compare — [`agent-unified-plan-doing-and-benchmarks-2026-05-08.md`](../analysis/agent-unified-plan-doing-and-benchmarks-2026-05-08.md) §Wave C; **Wave D (2026-05):** tooling окна калибровки, fingerprint guard, release-train compare — [`agent-engine-next-horizon-2026-05-13.md`](../analysis/agent-engine-next-horizon-2026-05-13.md) + [`agent-v3-quality-judge-calibration-2026-05.md`](../analysis/agent-v3-quality-judge-calibration-2026-05.md); historical detailed wave log: [`agent-engine-and-benchmarks-next-waves-2026-05-09.md`](../analysis/agent-engine-and-benchmarks-next-waves-2026-05-09.md) (stub); артефакты `current-agent-v3-quality-judge-{mini,pilot,holdout}.json`; см. [`eval/agent_v3_quality/README.md`](../../eval/agent_v3_quality/README.md) |

## Checklist: Agent v3 quality judge (Wave B / Wave C → stronger gate)

Использовать только после стабилизации **engineering** gate (`trace-review-v1`) и отдельного решения мейнтейнеров. Детальная программа Wave C (KPI, cadence, ReAct baseline policy): [`agent-unified-plan-doing-and-benchmarks-2026-05-08.md`](../analysis/agent-unified-plan-doing-and-benchmarks-2026-05-08.md) §Wave C. Спека judge: [`agent-v3-quality-llm-judge-benchmark-plan-2026-05-08.md`](../analysis/agent-v3-quality-llm-judge-benchmark-plan-2026-05-08.md) §9.

**Wave D (2026-05-10):** инструментарий (скрипты, тест fingerprint, `compare --release-train-gate`, embedded baseline, runbook) уже в репозитории — см. [`agent-engine-next-horizon-2026-05-13.md`](../analysis/agent-engine-next-horizon-2026-05-13.md) + [`agent-engine-feature-status-2026-05-13.md`](../analysis/agent-engine-feature-status-2026-05-13.md). Пункты ниже — что **подтверждается перед promotion** (live артефакты, пороги, стабилизация).
...
- [ ] **Wave D — fingerprint guard (machine):** `EXPECTED_JUDGE_PROMPT_FINGERPRINT` в [`eval/agent_v3_quality/contract.py`](../../eval/agent_v3_quality/contract.py) совпадает с `judge_prompt_fingerprint()`; при правке [`eval/agent_v3_quality/judge_prompt_v1.md`](../../eval/agent_v3_quality/judge_prompt_v1.md) обновить константу и начать новое окно стабилизации. Регрессия: `tests/scripts/test_judge_prompt_fingerprint_guard.py`.
- [ ] **Wave D — calibration window:** 6–10 кейсов из `judge_pilot`, 3 последовательных прогона одним fingerprint/моделью: [`scripts/run_agent_v3_quality_llm_calibration_subset.py`](../../scripts/run_agent_v3_quality_llm_calibration_subset.py) `--window` (операторский runbook: [`agent-v3-quality-judge-calibration-2026-05.md`](../analysis/agent-v3-quality-judge-calibration-2026-05.md)). Порог triage: `agreement_winner_rate >= 0.7` heuristic vs LLM на каждом прогоне (`--strict`).
- [ ] **Wave D — variance baseline:** после окон сохранить `eval/results/agent-v3-quality-judge-variance-baseline.json` (`--write-variance-baseline`); разброс `mean_delta` по трём прогонам ≤ **0.15** (см. артефакт и `variance` в JSON окна).
- [ ] **Wave D — frozen pilot baseline + release compare (не `decision_gate`):** зафиксированный снимок `judge_pilot`, например [`eval/results/baseline-agent-v3-quality-judge-pilot-embedded.json`](../../eval/results/baseline-agent-v3-quality-judge-pilot-embedded.json) (заменить на live LLM-judge baseline перед релизом при необходимости); регрессия на release train: `science-graphrag-agent-v3-quality-compare <baseline> <current-pilot> --release-train-gate` (пороги: `mean_delta` не ниже baseline − 0.10; `cases_with_any_branch_non_ok` ≤ 0).
...
### Stabilization window (рекомендация из spec + Wave C draft thresholds)

- [ ] Серия pilot-прогонов без «ломающей» judge-variance и без роста hard-fail у candidate относительно baseline (см. spec §9.2).
- [ ] На frozen `judge_pilot` (live subprocess): `cases_with_any_branch_non_ok == 0`, `all_passed == true`, `mean_delta >= 0` и pairwise не хуже baseline без явного write-up (см. unified plan §Wave C.1 — ориентиры до финальной калибровки).
