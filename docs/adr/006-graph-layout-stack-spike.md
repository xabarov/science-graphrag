...
- **2026-04-08 (decision):** **React Flow** POC merged: optional third viz mode **Flow** in [`GraphWorkspacePanel.jsx`](../../ui/src/components/graph/GraphWorkspacePanel.jsx), fed from `normalizeGraphPayload` / `capGraphForUi` via [`graphFlowAdapter.js`](../../ui/src/components/graph/graphFlowAdapter.js) and [`GraphFlowView.jsx`](../../ui/src/components/graph/GraphFlowView.jsx). Initial node positions reuse the same **circle world layout** as Canvas (`computeWorldLayout`); **no** force-directed simulation, **no** OSINT clustering. Default UX remains **Cards** / **Graph** (Canvas). Further options (Sigma, osint `useForceSimulation` port, iterative physics) stay backlog — see [`refactor-frontend.md`](../backlog/refactor-frontend.md).
