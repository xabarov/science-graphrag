...
- **Remaining (2026-05-14 follow-up):** `orchestrator_main_runner` теперь ~66 LoC; добавлены [`orchestrator_run_context.py`](../../scripts/live_check/trace_review/orchestrator_run_context.py), [`orchestrator_run_http_and_e2e.py`](../../scripts/live_check/trace_review/orchestrator_run_http_and_e2e.py), [`orchestrator_run_artifacts.py`](../../scripts/live_check/trace_review/orchestrator_run_artifacts.py) (~291 LoC — следующий кандидат на нарезку при росте). `compaction_turn_review.py` → тонкий entry + пакет [`compaction_review/`](../../scripts/live_check/compaction_review/) (`http_client`, `retry_policy`, `report_builder`, `run`). Дальше: дробить `compaction_review/run.py` / `orchestrator_run_artifacts.py` при росте; OD E2E audit.
