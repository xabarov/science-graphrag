...
### [PARTIAL] Introduce RoutePlan + QuestionFeatures (orchestration policy unification)
...
- **Issue:** Routing decisions are split across four layers — `narrow_deterministic_classify` / `rules_v0_classify`, `TurnPolicy` (`rules_v0` / `hybrid_v1` / `llm_v1`), supervisor LLM router, and ad-hoc `_should_force_*` / `_maybe_force_writer_*` heuristics in `supervisor_node`. Each new acceptance prompt (e.g. `v3_cv_fanout_dual_evidence`) requires a new substring marker tuple plus a new branch in `supervisor_node`. This is policy fragmentation — fixes accumulate as latches rather than as refinements of one model.
- **Proposal:** Introduce `RoutePlan` / `RouteStep` / `CompletionSignal` datatypes in `coordination/route_plan.py`; centralize substring/regex checks into `coordination/question_features.py` (the **only** place where prompt text is inspected); implement `build_route_plan(features, settings) -> RoutePlan` and have `supervisor_node` execute the plan (with LLM `maybe_replan(state, plan)` only on explicit `replan_signal`). Migrate existing force-rules (`workspace_dual_evidence_first_hop`, `retrieval_completion_*`) from `supervisor_node` into planner rules.
- **Done (2026-05-08):** delivered the contract layer + planner + supervisor decision split:
  ...
  - `science_graphrag/agent/coordination/question_features.py` — single substring/regex extractor with round-trip `to_dict` / `from_dict`; supersedes scattered marker tuples (the legacy `_DUAL_EVIDENCE_CATALOG_COMPARE_MARKERS` and the inline writer-handoff substring tables in `supervisor.py` are gone).
  - `science_graphrag/agent/coordination/route_planner.py` — `build_route_plan(features, turn_policy, settings)` and `planner_post_retrieval_handoff(features, tool_counts, completion_state)`. The legacy fallbacks `_maybe_force_writer_after_retrieval` / `_should_force_retrieval_first_hop_workspace_dual_evidence` are now thin shims over `extract_question_features` + `planner_post_retrieval_handoff`, so both code paths read the same marker tables.
  - `science_graphrag/agent/graph/supervisor_decisions.py` — pure `compute_first_hop_decision`, `compute_post_retrieval_handoff`, `compute_round_cap_decision`, `should_skip_llm_router`. `supervisor_node` now composes them; cached `QuestionFeatures` from `metadata.turn_policy.question_features` is reused (no per-supervisor-hop recomputation). Open-ended `default_supervisor_round_cap` plans deliberately defer to the LLM router after step 0; bounded `writer_after_first_hop_completion` plans deterministically pick writer.
  - Feature flags in `science_graphrag/config.py`: `agent_route_plan_enabled` (off by default), `agent_route_plan_post_retrieval_handoff_enabled`, `agent_supervisor_replan_only_llm_enabled`.
  - Routing log carries `decision_source=route_plan` for every plan-driven hop (both first-hop and `should_skip_llm_router` paths) — observable signal for dashboards and trace-review.
  - Regressions: `tests/agent/test_route_plan_and_features.py` (+27 tests including round-trip and open-ended skip), `tests/agent/test_per_tool_deadline_and_completion_state.py`, all 278 agent tests green; baseline doc — [`docs/analysis/orchestration-stabilization-baseline-2026-05-08.md`](../analysis/orchestration-stabilization-baseline-2026-05-08.md).
- **Remaining:** flip `agent_route_plan_enabled` to default-on after acceptance soak; flip `agent_route_plan_post_retrieval_handoff_enabled` to default-on; remove the two legacy compatibility shims (`_maybe_force_writer_after_retrieval`, `_should_force_retrieval_first_hop_workspace_dual_evidence`) once both flags are default-on; live trace-review measurement of `tool_loop_repeat_max` drop.
- **Plan ref:** [`docs/analysis/orchestration-stabilization-plan-2026-05-07.md`](../analysis/orchestration-stabilization-plan-2026-05-07.md) §3.1, §3.2, §4 Фаза 1.
- **Raised:** 2026-05-07 (orchestration stabilization plan)

### [PARTIAL] Single supervisor backbone with `single_agent_react_mode` flag
...
- **Issue:** `/v2/agent/query` serves two graphs depending on `Settings.agent_runtime`: `langgraph_research_v1` / `single_agent_research_v1` (ReAct, no `route_to_specialist` in trace) vs `langgraph_supervisor_v1` / `langgraph_supervisor_v3` (supervisor → specialists → writer). Dev `.env` is on the former, acceptance suites are written against the latter; a smoke against `:18787` does not verify what the supervisor route actually does (observed 2026-05-07 — HTTP returned `single_agent_research_v1` while in-container `agent.run` with `langgraph_supervisor_v3` showed the expected first hop). This is feature-flag fragmentation hiding behind one endpoint.
- **Done (2026-05-08):** ADR-029 merged — [`docs/adr/029-single-supervisor-backbone.md`](../adr/029-single-supervisor-backbone.md). Canonical backbone is `langgraph_supervisor_v3`; the simplified mode is now derived from the new `RoutePlan` (writer-only / single-step plans), not from a separate graph wiring. Legacy `langgraph_research_v1` remains as a test/offline fallback.
- **Remaining:** flip dev `.env` and `make dev-up` template default to `SCIENCE_GRAPHRAG_AGENT_RUNTIME=langgraph_supervisor_v3`; align operator runbooks under [`docs/runbooks/`](../runbooks/); after acceptance soak, default-on `agent_route_plan_enabled` and remove the legacy graph from `build_retrieval_graph`.
- **Plan ref:** [`docs/analysis/orchestration-stabilization-plan-2026-05-07.md`](../analysis/orchestration-stabilization-plan-2026-05-07.md) §3.3, §4 Фаза 2.
- **Raised:** 2026-05-07 (orchestration stabilization plan)

### [PARTIAL] Per-tool-call deadline + completion_state for graceful degradation
...
- **Issue:** Today only a global `agent_step_timeout_seconds` exists; one stuck LLM tool call eats the whole turn (observed 2026-05-07 — 240s deadline failed, 420s succeeded in ~27s on the same prompt). Salvage paths exist for writer drafts, quote candidates, and graph tool, but not for the last successful `paper_profile` / `find_works` bundle. Specialists do not surface explicit completion-readiness signals back to the supervisor.
- **Done (2026-05-08):**
  - Per-tool-call deadline cap in [`tool_execution_pipeline.py`](../../science_graphrag/agent/tool_execution_pipeline.py) (`agent_per_tool_call_timeout_seconds`, 0 disables). The deadline runs the tool batch in a **daemon thread** with `Thread.join(timeout=…)` rather than a `ThreadPoolExecutor` context manager — the latter would call `shutdown(wait=True)` on `__exit__` and silently re-block the supervisor on the very tool we just timed out (verified: the slow-tool test now wall-clocks at the 0.5s deadline, not the underlying 2s sleep). On expiry the batch surfaces a `permission_denied`-style ToolMessage with `error=per_tool_deadline_exceeded` and a `tool_execution.deadline` debug event.
  - `completion_state` field in `specialist_results_v3.merge` with `COMPLETION_STATE_VALUES` re-export from `coordination.route_plan.CompletionSignalId` (single source of literals); helper `annotate_completion_state` for specialists to stamp `minimal_bundle_ready` / `evidence_insufficient` / etc.
  - Planner consumes the typed marker (`planner_post_retrieval_handoff(..., completion_state=...)`) so explicit readiness short-circuits substring-feature gating in the supervisor.
  - Regressions: [`tests/agent/test_per_tool_deadline_and_completion_state.py`](../../tests/agent/test_per_tool_deadline_and_completion_state.py) (8 tests).
- **Remaining:** wire `annotate_completion_state` into `retrieval_agent` / `graph_agent` exit paths (currently only the data shape and planner hookup are in place); extend `runtime.py` salvage to use the last successful `find_works`/`paper_profile` bundle on global deadline; live trace-review evidence that forced deadline now returns partial answer + warning.
- **Plan ref:** [`docs/analysis/orchestration-stabilization-plan-2026-05-07.md`](../analysis/orchestration-stabilization-plan-2026-05-07.md) §3.4, §4 Фаза 3.
- **Raised:** 2026-05-07 (orchestration stabilization plan)

### [PARTIAL] Split `supervisor_node` into `supervisor_decisions` module
- **Area:** `science_graphrag/agent/graph/supervisor.py` (inner closure `supervisor_node`)
- **Issue:** `supervisor_node` triggers pylint R0911 (9 returns) / R0912 (13 branches) / R0915 (57 statements) in one closure that mixes coordinator-gate handling, force-handoff rules, route_hint reading, semantic_fast_route, round cap, LLM routing call, and post-LLM normalization. Any routing change touches a place where five other concerns also live, which is why each acceptance fix lands as another latch rather than a refinement.
- **Done (2026-05-08):** extracted `compute_first_hop_decision`, `compute_post_retrieval_handoff`, `compute_round_cap_decision`, `should_skip_llm_router` into [`science_graphrag/agent/graph/supervisor_decisions.py`](../../science_graphrag/agent/graph/supervisor_decisions.py). `supervisor_node` now composes them via a small orchestration block and emits routing-log entries. Inline substring tables (`_DUAL_EVIDENCE_CATALOG_COMPARE_MARKERS` and quote/catalog markers in `_maybe_force_writer_after_retrieval`) were retired in favour of `coordination.question_features`, dropping `supervisor.py` from 634 to 563 LoC. Existing `tests/agent/test_supervisor_routing.py` (24 tests) and the new `test_route_plan_and_features.py` are green without logic changes; legacy heuristics retained as thin shims behind feature flags as fallback.
- **Remaining:** `supervisor.py` is still ~563 LoC (target was < 400). Planned next: move the LLM-routing block (`ROUTING_PROMPT`, `_build_supervisor_route_messages`, `invoke_chat_gated` call site, post-LLM normalization) into `supervisor_decisions.maybe_replan` once `agent_supervisor_replan_only_llm_enabled` defaults to true. Until then keep the single-place ownership of the LLM-call surface for clarity.
- **Plan ref:** [`docs/analysis/orchestration-stabilization-plan-2026-05-07.md`](../analysis/orchestration-stabilization-plan-2026-05-07.md) §4 Фаза 4.1.
- **Raised:** 2026-05-07 (orchestration stabilization plan)

### [PARTIAL] Reduce supervisor route churn before writer handoff
...
- **Done (2026-05-08):** delivered the orchestration plumbing for a deterministic handoff: planner-side `planner_post_retrieval_handoff(..., completion_state=...)` short-circuits to writer when `minimal_bundle_ready` / `writer_ready` is reported; supervisor reads `metadata.turn_policy.route_plan` for first-hop decisions; `agent_route_plan_post_retrieval_handoff_enabled` switches `_maybe_force_writer_after_retrieval` to the planner path.
- **Remaining:** producer side — wire `annotate_completion_state(..., "minimal_bundle_ready")` into retrieval/graph specialists at the right call sites; live measurement of `tool_loop_repeat_max` on the default dev-v3 trace-review (target ≤ 4 from §6 of the stabilization plan); flip `agent_route_plan_post_retrieval_handoff_enabled` to default-on after acceptance soak.
