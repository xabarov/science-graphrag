...
- **Progress (2026-04-27):** Phase 2 slice landed in UI: `experiment` / `runMode` / `pack` query handoff, catalog CTAs into Run Lab, client-side grouped batch (`benchmarkRunGroup.js` + [`useBenchmarkRunGroup.js`](../../ui/src/pages/BenchmarkPage/useBenchmarkRunGroup.js) composed from [`useRunTab.js`](../../ui/src/pages/BenchmarkPage/useRunTab.js)), `benchmark:lastRunGroup` / recent compare setups in `localStorage`, Analysis handoff via `run` + `runs` query. **Follow-up:** optional backend `run-group` API if frontend orchestration becomes heavy; Phase 3 matrix summaries on Analysis; if `useRunTab` grows again, split single-run polling into a thin hook.
- **Raised:** 2026-04-27
