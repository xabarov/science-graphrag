...
Добавить в `science-graphrag` полноценную страницу просмотра и запуска бенчмарков, чтобы пользователь мог:
...
План опирается на текущий MVP `/benchmark` в `science-graphrag` и расширяет его до режима benchmark workbench.
...
## UX specifics for `science-graphrag`
