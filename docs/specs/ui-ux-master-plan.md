...
- **URL `ask_session`:** `TRACEABILITY_QUERY_KEYS.askSession` в `ui/src/routing/queryKeys.js`; синхронизация в `AskPage` / `AskTab` + `AskPanel` (`urlSessionId` / `onUrlSessionIdChange`); сброс при смене вкладки workspace вне Ask; `mergeTraceabilityParams` в `traceabilityState.js` сохраняет параметр при навигации (например Graph → Ask).
