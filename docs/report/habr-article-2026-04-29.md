...
В Phoenix по одному запросу видно «лесенку» из нескольких `chat_completion` и вызовов инструментов; тот же упрощённый пример цепочки вызовов приложен к репозиторию в машиночитаемом виде (JSON), чтобы не перегружать текст статьи.
