...
**Зафиксированные числа:** сырые JSON и текстовые манифесты окон публикации (**`habr-window-2026-05`** … **`habr-window-2026-07`**) лежат в [каталоге бенчмарк-артефактов на GitHub](https://github.com/xabarov/science-graphrag/tree/main/eval/results). Дальше по тексту повторяются только имена волн и коммитный контекст, без файловых путей в стиле «внутренней канализации» репозитория.
...
- Извлечение утверждений на перефразированном эталоне: macro-F1 **`0.224`** (пилот) и **`0.190`** (holdout) после **итерации v2** промпта (май 2026; см. §4.1). До промпта на тех же сплитах **`0.217` / `0.187`** — волна **`habr-window-2026-05`**. **Июнь 2026:** офлайн пост-дедуп на майских предсказаниях — **`0.223605` / `0.190398`**; **новый live** production BT6 с дедупом в пути — **~`0.249` / ~`0.219`**; сводка и файлы — **`habr-window-2026-06`** в каталоге артефактов по ссылке выше. *Как считаю F1:* для каждого кейса \(F_1 = 2PR/(P+R)\) из `claim_precision` / `claim_recall`, затем **среднее по кейсам** (macro по кейсам).
...
- **Публикационные прогоны.** Для текста статьи фиксирую дату, коммит и JSON-снимки в [репозитории](https://github.com/xabarov/science-graphrag/tree/main/eval/results) (первая зафиксированная волна — **`habr-window-2026-05`**). Актуальный план измерений и нарратива для статьи — [`habr-article-narrative-and-measurement-plan-2026-07.md`](../analysis/habr-article-narrative-and-measurement-plan-2026-07.md).
...
Чтобы связать «`p95 ≈ 26` секунд» с доказательством, в репозитории лежит **иллюстративный** JSON-эксцерт иерархии span’ов (`llm.chat_completion` → инструменты → снова `llm…`) — [machine-readable пример в GitHub](https://github.com/xabarov/science-graphrag/blob/main/docs/analysis/phoenix-trace-multistep-excerpt.json) (структура как у OTLP-трейсов, попадающих в Phoenix; не один конкретный прод-trace).
...
**Про holdout на пять кейсов (`claims_holdout_v1`).** Это намеренно узкий «замок» от подгонки промпта: macro-F1 в TL;DR — **среднее по кейсам**, и при \(n=5\) отдельные кейсы сильно тянут среднее. На июньском live-снимке holdout (**`habr-window-2026-06`**) per-case \(F_1\) по кейсам лежит примерно между **0.0** и **0.4** (медиана порядка **0.25**); подробности — в JSON этой волны в [каталоге артефактов](https://github.com/xabarov/science-graphrag/tree/main/eval/results). Доверительные интервалы здесь не притворяемся считать: важно другое — **пилот и holdout не смешиваются** в одну «головную цифру» (см. Appendix A в [контракте claims / BT6](https://github.com/xabarov/science-graphrag/blob/main/docs/benchmarks/ontology-claims-benchmark-v1.md)).
...
.venv/bin/science-graphrag-claims-paraphrase-benchmark \
  ...
  --json-out ./repro-holdout.json
```

Зафиксированный снимок для сравнения с текстом статьи — коммит из манифеста волны **`habr-window-2026-06`** в [каталоге артефактов на GitHub](https://github.com/xabarov/science-graphrag/tree/main/eval/results).
...
**Перефразированный эталон** — каждое эталонное утверждение **переписано своими словами** и проходит ограничение на перекрытие с исходником: нельзя выиграть, просто склеив подстрочник из PDF. Метрика спрашивает: узнаёт ли модель **тот же смысл**, что зафиксировал автор эталона, если формулировки разные. На полном прогоне пилота (15 кейсов, тир `claims_pilot_v2`) — macro-F1 **`0.224`** после итерации v2 промпта (май 2026); на отложенной выборке (5 кейсов, `claims_holdout_v1`) — **`0.190`**. Июнь 2026: (1) **офлайн re-score** майских предсказаний с пост-дедупом — **`0.223605`** / **`0.190398`**; (2) **новый live** прогон production BT6 с дедупом в пути — **~`0.249`** / **~`0.219`** macro-F1 по кейсам — волна **`habr-window-2026-06`**. До одной оси изменений (только текст SYSTEM_BENCHMARK для claims) на тех же сплитах было **`0.217` / `0.187`** — волна **`habr-window-2026-05`**. Методология сплитов и скоринга — Appendix A в [контракте claims / BT6 на GitHub](https://github.com/xabarov/science-graphrag/blob/main/docs/benchmarks/ontology-claims-benchmark-v1.md).

*Пример одного тезиса из набора по DETR* (идентификатор в эталоне: `detr_bipartite_matching_no_nms`; после волны **gold-v2** формулировка в эталоне приближена к тому, как production-экстрактор уже пишет «set-based loss + bipartite matching + без NMS»):
...
| Сплит | Macro-F1 до (baseline) | Macro-F1 после v2 (май) | Июнь: offline dedupe на майских preds | Июнь: live production + dedupe | Где смотреть снимки |
|------|------------------------|-------------------------|----------------------------------------|----------------------------------|---------------------|
| Пилот (15 кейсов) | **0.217** | **0.224** | **0.223605** | **~0.249** | волны **`habr-window-2026-05`** (май) и **`habr-window-2026-06`** (июнь offline / live) в [каталоге артефактов](https://github.com/xabarov/science-graphrag/tree/main/eval/results) |
| Holdout (5 кейсов) | **0.187** | **0.190** | **0.190398** | **~0.219** | то же |

Прирост небольшой, зато честно сопоставим: один протокол, зафиксированные файлы, дата и сводка — манифесты **`habr-window-2026-05`** и **`habr-window-2026-06`** в [каталоге артефактов](https://github.com/xabarov/science-graphrag/tree/main/eval/results). Разбор типичных промахов — отдельная заметка той же майской волны в том же каталоге.

**Волна gold-v2 (май 2026, фикстуры `schema_version: 3`).** На **пяти** строках `expected_claims` (DINO ×2, DETR ×1, EfficientDet ×2) я подтянул формулировки к тому, что реально печатает production-экстрактор в live BT6, не меняя `claim_id` и не трогая `match_mode`. Отдельный повторный прогон зафиксирован волной **`habr-window-2026-07`**. **Headline-цифры в TL;DR остаются на июньском live-снимке**: один свежий прогон после правки эталона сильно смешивает эффект gold с **дисперсией LLM** и иногда с дорожкой дистракторов; пилотное среднее в этом артефакте почти не изменилось, а holdout «прыгнул» — честнее трактовать gold-v2 как **аудит эталона**, а не как гарантированный +macro без мульти-сидовой агрегации.

Компактная выжимка по трём разобранным кейсам из майского разбора ошибок (**`habr-window-2026-05`**):
...
*Пример, где промпт v2 реально помог (пилот, macro-F1 по кейсу):* `corpus_cascade_rcnn_v2` — с **~0.267** (baseline) до **~0.414** после v2 на том же сплите (сравнение baseline vs iter v2 в JSON волны **`habr-window-2026-05`** в [каталоге артефактов](https://github.com/xabarov/science-graphrag/tree/main/eval/results)).
...
**Качество извлечения сущностей (macro по слотам):** подробная markdown-таблица по слотам вынесена в [приложение на GitHub](https://github.com/xabarov/science-graphrag/blob/main/docs/report/habr-article-appendix-metrics-table-2026-06.md), чтобы укоротить §6; ниже остаются сводки по графу, сквозному агенту и retrieval.
...
- **Живой mini-tier (`live_corpus_mini`, июнь 2026):** три кейса на пилотном корпусе (векторный поиск + отпечатки чанков) — **3/3** PASS; снимок и строка в манифесте — **`habr-window-2026-06`** в [каталоге артефактов](https://github.com/xabarov/science-graphrag/tree/main/eval/results). Контрактный smoke без реального Qdrant — отдельный JSON той же волны (mock runtime, не смешивать с live headline).
...
Дополнительно — для перефразированного эталона в репозитории есть диагностический скрипт paraphrase-диагностики, который раскладывает кейсы по таксономии «полный recall / частичный / ноль» и **отдельно** считает lexical overlap между эталоном и предсказаниями (порог покрытия токенов эталона ≥ 0.35; это **не** то же самое, что `embedding_sim` в BT6). Сводка по итерации v2 (пилот) лежит в markdown-артефакте волны **`habr-window-2026-05`** в [каталоге артефактов](https://github.com/xabarov/science-graphrag/tree/main/eval/results); средние P / R / F1 по этой ослабленной калибровке порядка **`0.18 / 0.07 / 0.10`** — то есть слабость извлечения **не сводится** к одному только «жёсткому» paraphrase-матчингу.

### Бенчмарк действительно различает модели: три прогона на одной выборке

*Примечание:* таблица ниже — мультимодельный срез на отложенной выборке утверждений (**5 кейсов**, тот же протокол BT6, что и в остальной статье). Прогоны зафиксированы **июль 2026** (три модели, один эталон). **Recall / Precision** — среднее по кейсам; **F1** — среднее per-case (\(2PR/(P+R)\) для каждого кейса, затем среднее по пяти), в той же логике, что macro-F1 по кейсам в TL;DR. Главные headline-цифры по пилот/holdout в шапке статьи и в §4.1 относятся к **июньскому live**-снимку основной линии; с этой таблицей их **не смешивать** (другая свёртка и другой смысл «головной» строки в TL;DR).
...
|---|---|---|---|---|
| `mistralai/mistral-small-3.2-24b-instruct` (база) | **0.250** | **0.164** | **0.156** | 1 / 5 |
| `deepseek/deepseek-v3.2` | **0.350** | **0.194** | **0.218** | 1 / 5 |
| `moonshotai/kimi-k2.6` | **0.250** | **0.095** | **0.128** | 1 / 5 |

Что здесь видно. На этом срезе **DeepSeek** даёт наибольший средний F1 per-case (**0.218**) при самом высоком mean recall (**0.35**). **Mistral** — **0.156** F1 при **0.25** / **0.164** mean R/P. **Kimi** — **0.128** F1 при **0.25** / **0.095**; число кейсов с нулевым per-case F1 совпало для всех трёх (**1/5**). Разброс по F1 сохраняется — значит бенчмарк по-прежнему не «выравнивает всех в одну цифру».

Главный методический вывод: если бы мы мерили эти три модели **только по цитатному эталону**, у всех трёх получился бы recall в районе единицы — и мы бы не отличили рабочий вариант от провального. На перефразированном эталоне разница между лучшим и худшим средним F1 на этом срезе порядка **1.7×** (**0.218** vs **0.128**). Дальше можно осмысленно сравнивать промпт-варианты, версии экстрактора и провайдеров.
...
**Не путать «сквозной 10/10» с узким `agent_tools_mini`.** В тире `agent_tools_mini` gold жёстко фиксирует `expected_tool_sequence` (часто первый шаг — `idea_search`). На июньском live-прогоне при узкой полосе скоринга для `tool_search` формально проходит **1/10** кейсов, хотя ответы часто остаются grounded: доминирует **несовпадение маршрута** с эталоном (`find_works` / `workspace_inspect` вместо ожидаемого первого инструмента, лишние циклы `paper_profile`). Полная таблица классов провалов по сохранённому `tool_trace` (без дополнительного вызова LLM) и текст разбора лежат **в репозитории проекта** вместе с остальными материалами воспроизводимости.

В Phoenix по одному запросу видно «лесенку» из нескольких `chat_completion` и вызовов инструментов; тот же упрощённый пример цепочки — [JSON в репозитории на GitHub](https://github.com/xabarov/science-graphrag/blob/main/docs/analysis/phoenix-trace-multistep-excerpt.json), чтобы не перегружать текст статьи.
