...
> Автор: Роман (`<@nickname>`) · Исходники и материалы проекта: **[github.com/xabarov/science-graphrag](https://github.com/xabarov/science-graphrag)**
...
- Извлечение утверждений на перефразированном эталоне: macro-F1 **`0.224`** (пилот, 15 кейсов) и **`0.190`** (holdout, 5 кейсов) после **итерации v2** промпта (май 2026; см. §4.1) — **намеренно низко относительно «удобных» эталонов**, и про это будет отдельный раздел. Для сравнения: до правки того же протокола **`0.217` / `0.187`** на тех же сплитах — артефакты в `eval/results/habr-window-2026-05-*.json`.

Главная мысль статьи — **не сам GraphRAG, а программа честной оценки** вокруг него. Если ваши контрольные наборы устроены удобно, метрики выглядят красиво — но не значат ничего. Я специально оставлю в тексте «неудобные» цифры и расскажу, почему это полезнее, чем единица.
...
**Перефразированный эталон** — каждое эталонное утверждение **переписано своими словами** и проходит ограничение на перекрытие с исходником: нельзя выиграть, просто склеив подстрочник из PDF. Метрика спрашивает: узнаёт ли модель **тот же смысл**, что зафиксировал автор эталона, если формулировки разные. На полном прогоне пилота (15 кейсов, тир `claims_pilot_v2`) — macro-F1 **`0.224`** после итерации v2 промпта (май 2026); на отложенной выборке (5 кейсов, `claims_holdout_v1`) — **`0.190`**. До одной оси изменений (только текст SYSTEM_BENCHMARK для claims) на тех же сплитах было **`0.217` / `0.187`** — см. заморозку в `eval/results/habr-window-2026-05-manifest.txt`. Методология сплитов и скоринга — Appendix A в [`docs/benchmarks/ontology-claims-benchmark-v1.md`](../benchmarks/ontology-claims-benchmark-v1.md).

*Пример одного тезиса из набора по DETR* (идентификатор в эталоне: `detr_bipartite_matching_no_nms`; текст эталона — из `corpus_detr_v2` в фикстурах бенчмарка):

| Что проверяем | Текст |
|---|---|
| **Эталон** (как должно быть сформулировано утверждение в ответе экстрактора) | *«Bipartite matching between predicted and ground-truth boxes during training enforces a unique prediction per object, which removes the need for non-maximum suppression at inference.»* |
...
| Сплит | Macro-F1 до (baseline) | Macro-F1 после v2 | JSON артефакты |
|------|------------------------|-------------------|----------------|
| Пилот (15 кейсов) | **0.217** | **0.224** | `eval/results/habr-window-2026-05-baseline-pilot.json` → `habr-window-2026-05-iter-v2-pilot.json` |
| Holdout (5 кейсов) | **0.187** | **0.190** | `eval/results/habr-window-2026-05-baseline-holdout.json` → `habr-window-2026-05-iter-v2-holdout.json` |

Прирост небольшой, зато честно сопоставим: один протокол, зафиксированные файлы, дата и сводка в `eval/results/habr-window-2026-05-manifest.txt`. Примеры промахов — `eval/results/habr-window-2026-05-failure-analysis.md`.
...
**Качество извлечения сущностей (macro по слотам):**

| Слот | Метрика | Значение |
|---|---|---|
| Заголовок | ROUGE-L | 1.000 |
| Аннотация (vs префикс эталона) | ROUGE-L | 0.119 |
| Аннотация — покрытие токенов префикса | — | 0.926 |
| Имена авторов | F1 | **0.981** |
| Аффилиации | F1 | 0.951 |
| Ссылки на arXiv (выборка из эталона) | F1 | 0.957 |
| Ссылки на DOI (выборка из эталона) | F1 | 0.933 |
| Методы (семантика для графа) | F1 | **0.601** |
| Датасеты (семантика для графа) | F1 | **0.792** |
| Концепты | F1 | 0.880 |
| Темы | F1 | 1.000 |
| Утверждения (перефразированный эталон), пилот | F1 | **0.224** (iter. v2, май 2026) |
| Утверждения (перефразированный эталон), отложенная выборка | F1 | **0.190** (iter. v2, май 2026) |
...
Дополнительно — для перефразированного эталона у меня есть отдельный диагностический скрипт, который раскладывает кейсы по таксономии «полный recall / частичный / ноль» и **отдельно** считает lexical overlap между эталоном и предсказаниями (порог покрытия токенов эталона ≥ 0.35). Даже при ослаблении критерия macro P / R / F1 порядка `0.04 / 0.15 / 0.07` — то есть слабость извлечения **не объясняется только строгим матчингом**.

### Бенчмарк действительно различает модели: три прогона на одной выборке

*Примечание:* таблица ниже — **отдельный** мультимодельный срез на holdout (исторический прогон в `eval/results/multimodel/`); актуальные пары «baseline / итерация v2» на тех же тирах для основной линии статьи — в таблице §4.1 выше и в `eval/results/habr-window-2026-05-*.json`.
...
**Фикс.** Сделал отдельный, перефразированный эталон — формулировки утверждений переписаны и ограничены по перекрытию с исходным текстом (порог покрытия токенов исходника). На нём recall провалился до `0.25–0.33`, а F1 — до `0.15–0.19`. Это и есть честная цифра.
