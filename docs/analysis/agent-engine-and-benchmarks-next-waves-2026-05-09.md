...
| **Wave H rollout** | **Live** long-thread acceptance для `agent_llm_full_history_compact_enabled` + idle `agent_tool_message_microcompact_time_trigger_enabled`. **Preflight:** для `suite=acceptance` обязателен непустой `AGENT_LIVE_WORKSPACE_ID` / `--workspace-id` (fanout); paired compare — только baseline/candidate с **одинаковым** workspace и флагами. См. [`wave-h-rollout-decision-2026-05-12.md`](./wave-h-rollout-decision-2026-05-12.md) §«Trace-review / fanout», runbook [`agent-trace-review-sop.md`](../runbooks/agent-trace-review-sop.md) §1 и §9. | P2 | context |
