...
- **Phase 6.C pending:** extractors для остальных 8 layers (contradictions_v1, concept_topic_v2, dedup × 5, multihop_v2, workspace_scoped_live, hybrid_ablation_v2, agent_tools_live, idea_assist_live); финальный промо всех pack'ов в `human_spot_checked`.
- **Phase 6.D pending (предложен):** embedding-based scorer (sentence-transformers cosine similarity ≥ 0.7) поверх существующего token+char-gram matcher. Снимет потолок recall с 50% до >75% (особенно для семантически идентичных, но лексически расходящихся пар вроде `cascade_rcnn_v2`), разблокирует auto-promo `validation_status: llm_dual_validated`.
