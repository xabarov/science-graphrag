...
- **Update 2026-04-26:** шаг **1** закрыт; **п.2 — DONE** (unbounded Qdrant tag); **п.3 — DONE**; **п.4 — артефакты обновлены** (BT2/BT4/BT5 + trust baseline); acceptance §10.4 по BT2/BT4 пока не выполнен (качество ответов / `mrr_delta`). Дальше: **blob path для API** (§10.1.a) + embeddings/retrieval tuning (ADR-021) или политика §10.3 для hybrid.
