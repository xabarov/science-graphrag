...
|-------|------|-----------------------------|-------------|
| **A — Quick wins** | Cheaper ticks without structural rewrites | Hours to ~1 day | Tune Barnes–Hut `theta`; warm start / avoid unnecessary `simulationSignature` cold restarts; optional **throttle** commits to React (e.g. apply `setNodes` every 2–4 rAF frames if motion remains acceptable). |
