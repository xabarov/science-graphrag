...
- **UI:** [`authorSemanticProjection.js`](../../ui/src/components/graph/authorSemanticProjection.js) — **`projectAuthorSemanticGraph`** is pass-through; **`collectAuthorAggregatorExpandEndpoints`** remains for tests / legacy graphs; [`prefetchAuthorAggregatorExpansions`](../../ui/src/components/graph/workspaceGraphMergePrefetch.js) is a no-op (no server `Aggregator` placeholders).
