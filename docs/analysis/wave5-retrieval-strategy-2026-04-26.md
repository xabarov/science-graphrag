...
2. **BT6 claims paraphrase — production extractor**  
   ...
   **Next steps for `trust_signal.runtime_mode=live`:** rerun `science-graphrag-claims-paraphrase-benchmark --extractor production` on `claims_paraphrase_bt6_mini` then pilot; optional model with JSON mode / `extraction_llm_mode=json` if the provider supports it.
