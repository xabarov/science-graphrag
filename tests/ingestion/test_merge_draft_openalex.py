...
def test_merge_draft_prefer_enriched_overwrites_year_and_venue_when_enriched() -> None:
    base = WorkDraft(
        title="Example",
        normalized_title="example",
        doi="10.1234/example",
        publication_year=2020,
        venue_name="Base Venue",
        work_type=WorkType.UNKNOWN,
        ingestion_confidence=0.4,
    )
    enriched = WorkDraft(
    ...
    merged = merge_draft_prefer_enriched(base, enriched)
    assert merged.publication_year == 2021
    assert merged.venue_name == "OpenAlex Venue"
