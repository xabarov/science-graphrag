...
def test_react_chat_response_budget_cutoff_triggers_when_below_reserve() -> None:
    ...
    assert any(isinstance(e, dict) and e.get("type") == "budget_stop_decision" for e in ev)
...
def test_react_budget_stop_decision_can_be_disabled() -> None:
    settings = Settings(
        agent_step_timeout_seconds=60.0,
        agent_min_llm_hop_reserve_seconds=1.0,
        agent_budget_stop_reasoning_enabled=False,
    )
    state = {
        "metadata": {
            "agent_response_deadline_perf_start": perf_counter() - 100.0,
    ...
    assert out is not None
    ev = out.get("debug_events") or []
    assert not any(isinstance(e, dict) and e.get("type") == "budget_stop_decision" for e in ev)
