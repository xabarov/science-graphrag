...
def test_russian_user_message_padded_tool_names_still_normalize() -> None:
    ...
    msg = AIMessage(
        ...
        tool_calls=[
            {"name": "  summarize_workspace  ", "id": "a", "args": {}},
            {"name": " idea_search ", "id": "b", "args": {"query": "тест"}},
        ],
    )
    out = normalize_ai_message_tool_calls(msg)
    assert out.tool_calls[0]["name"] == "summarize_workspace"
    assert out.tool_calls[1]["name"] == "idea_search"


def test_normalize_tool_call_name_strips_padding() -> None:
    assert normalize_tool_call_name("  workspace_overview  ") == "workspace_overview"
    assert normalize_tool_call_name(" summarize_workspace ") == "summarize_workspace"
...
def test_normalize_ai_message_tool_calls() -> None:
    msg = AIMessage(
        content="",
        tool_calls=[
            {"name": "  workspace_overview  ", "id": "1", "args": {}},
            {"name": " idea_search ", "id": "2", "args": {"q": "x"}},
        ],
    )
    out = normalize_ai_message_tool_calls(msg)
    assert out.tool_calls[0]["name"] == "workspace_overview"
    assert out.tool_calls[1]["name"] == "idea_search"
...
def test_collect_tool_trace_uses_normalized_names() -> None:
    ...
    msg = AIMessage(
        content="",
        tool_calls=[{"name": " workspace_overview ", "id": "tc1", "args": {}}],
    )
    follow = ToolMessage(content="{}", tool_call_id="tc1", name="workspace_overview")
    state: AgentState = {"messages": [msg, follow]}
    traces = collect_tool_trace(state)
    assert any(t.get("tool") == "workspace_overview" for t in traces)


def test_build_normalized_tool_node_executor_invokes_with_trimmed_names() -> None:
    @tool
    def workspace_overview(workspace_id: str = "") -> str:
        """Stub workspace overview for ToolNode lookup."""
        _ = workspace_id
        return '{"ok": true}'
    ...
    ai = AIMessage(
        content="",
        tool_calls=[{"name": "  workspace_overview  ", "id": "call-1", "args": {}}],
    )
    state: AgentState = {"messages": [ai], "budget_remaining": 5}
    result = executor(state)
    messages = list(result.get("messages") or [])
    assert any(getattr(m, "name", None) == "workspace_overview" for m in messages)
