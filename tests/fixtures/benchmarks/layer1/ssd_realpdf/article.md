...
providing a unified framework for both training and inference. For300× 300 in-
...
experiments: 77.2% mAP for300×300 input and 79.8% mAP for512×512 input on VOC2007.
...
detection (59 FPS with mAP 74.3% on VOC2007test, vs. Faster R-CNN 7 FPS with
...
both the shape offsets and the confidences for all object categories ( (c1,c 2,··· ,c p)).
...
will call the base network 2. We then add auxiliary structure to the network to produce
...
architecture in Fig. 2. For a feature layer of size m×n withp channels, the basic el-
...
coordinates. At each of them×n locations where the kernel is applied, it produces an
...
Conv: 1x1x128
Conv: 3x3x256-s1
Detections:8732 per ClassClassifier : Conv: 3x3x(4x(Classes+4))
512
448
448
3
...
k at a given location, we computec class scores and the 4 offsets relative to the original
...
p exp(cp
i ) (3)
and the weight termα is set to 1 by cross validation.
...
pool5 from 2× 2−s2 to 3× 3−s1, and use the `a trous algorithm [18] to fill the
...
confidences. We set default box with scale 0.1 on conv433. We initialize the parameters
for all the newly added convolutional layers with the ”xavier” method [20]. For conv43,
...
accurate than Fast R-CNN. When we train SSD on a larger512× 512 input image, it is
...
3 For SSD512 model, we add extra conv122 for prediction, set smin to 0.15, and 0.07 on conv4 3.
...
”07”: VOC2007 trainval, ”07+12”: union of VOC2007 and VOC2012trainval.
”07+12+COCO”: first train on COCOtrainval35k then fine-tune on 07+12.
...
0.68
0.91
0.970.93airplane bicycle bird boat cat chair table
SSD512: BBox Area
XTT M WXW XTT M WXW XTT M WXW XTT M WXW XTT M WXW XTT M WXW XTT M WXW
0
0.2
...
except that we use VOC2012trainval and VOC2007 trainval and test (21503
...
the results of our SSD300 and SSD512 4 model. We see the same performance trend
as we observed on VOC2007 test. Our SSD300 improves accuracy over Fast/Faster R-
CNN. By increasing the training and testing image size to512× 512, we are 4.5% more
...
data: ”07++12”: union of VOC2007trainval and test and VOC2012 trainval.
”07++12+COCO”: first train on COCOtrainval35k then fine-tune on 07++12.
3.4 COCO
...
dataset [16]. We train a SSD300 model using the ILSVRC2014 DETtrain and val1
as used in [22]. We first train the model with10−3 learning rate for 320k iterations, and
...
0.32
0.70
0.940.970.98airplane bicycle bird boat cat chair table
SSD512*: BBox Area
...
In: CVPR. (2015)
11. Hariharan, B., Arbel ́aez, P., Girshick, R., Malik, J.: Hypercolumns for object segmentation
