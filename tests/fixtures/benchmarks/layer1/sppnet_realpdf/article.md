...
Existing deep convolutional neural networks (CNNs) require a fixed-size ( e.g., 224×224) input image. This require-
...
Figure 1: Top: cropping or warping to fit a fixed
...
2 D EEP NETWORKS WITH SPATIAL PYRA -
MID POOLING
...
are generated by some filters of the conv 5 layer. Fig-
...
conv5 layer, and conv 5 is the last convolutional layer.
...
window pooling, where the window sizewin =⌈a/n⌉
...
layer (fc 6) will concatenate the l outputs. Figure 4
...
whose feature map size of conv 5 is 13×13, so the
pool3×3, pool 2×2, and pool 1×1 layers will have 3 ×3,
...
size after conv 5 isa×a = 10×10 in this case. Then we
...
four corners from the entire image 1. The data are aug-
...
after conv2 and conv3 (instead of after conv 1 and
...
filter number (512) is used in conv 3 and the fol-
...
where conv 3 to conv 7 have the same structures.
...
than its no-SPP counterpart, because its fc 6 layer has
...
of the image.
3.1.3 Multi-size T raining Improves Accuracy
...
3.1.5 Multi-view T esting on Feature Maps
...
Table 6: Classification mAP in Pascal VOC 2007. For SPP-net, the pool 5/7 layer uses the 6×6 pyramid level.
(a) (b) (c) (d)
...
Table 7: Classification accuracy in Caltech101. For SPP-net, the pool 5/7 layer uses the 6×6 pyramid level.
...
ing [7]. Since our features are pooled from the conv 5
...
pooled features after conv 5, and the fc 6,7 layers and
a new 21-way (one extra negative category) fc 8 layer
...
of the pool 5 features used in [7]). The windows used
...
tures including conv 5. DetectorNet [40] trains a deep
...
Table 12: Detection results on VOC 2007 using model combination. The results of both models use “ftfc 7 bb”.
...
GPU time of computing the 4,096-d fc 7 features is
...
constrained linear coding for image classification,” in CVPR,
2010.
[19] F. Perronnin, J. S ́anchez, and T. Mensink, “Improving the fisher
...
training with sparse coding and vector quantization,” inICML,
2011.
