...
standard detection tasks likePASCAL VOC and COCO. Us-
...
recall of 81%. With anchor boxes our model gets69.2 mAP
...
we get with various choices fork. We find thatk = 5 gives a good
...
We run k-means for various values ofk and plot the av-
...
region proposal networks the network predicts valuestx and
ty and the (x,y ) center coordinates are calculated as:
x = (tx∗wa)−xa
y = (ty∗ha)−ya
For example, a prediction oftx = 1 would shift the box
...
offset from the top left corner of the image by (cx,c y) and
...
we changed the resolution to416×416. However, since our
...
Table 3: Detection frameworks on P ASCAL VOC 2007.
...
for only 10 epochs and starting at a learning rate of10−3. At
...
ters each followed by a final1× 1 convolutional layer with
