...
2.2 T ransformers and Parallel Decoding
...
Let us denote by y the ground truth set of objects, and ˆ y ={ˆyi}N
i=1 the
...
elementsσ∈ SN with the lowest cost:
ˆσ = arg min
σ∈SN
N∑
i
Lmatch(yi, ˆyσ(i)), (1)
whereLmatch(yi, ˆyσ(i)) is a pair-wise matching cost between ground truthyi and
...
commensurable toLbox(·,·) (described below), and we observed better empirical
performances.
...
λiouLiou(bi,ˆbσ(i)) +λL1||bi− ˆbσ(i)||1 where λiou,λ L1∈ R are hyperparameters.
...
32, W0
32 .
T ransformer encoder. First, a 1x1 convolution reduces the channel dimension
of the high-level activation map f from C to a smaller dimension d. creating a
new feature mapz0∈ Rd×H×W . The encoder expects a sequence as input, hence
we collapse the spatial dimensions ofz0 into one dimension, resulting in ad×HW
...
they all have the same dimensions (H0,W 0) as the largest image of the batch.
...
T ransformer decoder. The decoder follows the standard architecture of the
...
theN input embeddings must be different to produce different results. These in-
...
TheN object queries are transformed into an output embedding by the decoder.
...
epochs.
T echnical details. We train DETR with AdamW [26] setting the initial trans-
...
having lower AP S but greatly improved AP L. We use torchscript Faster R-CNN and
DETR models to measure FLOPS and FPS. Results without R101 in the name corre-
spond to ResNet-50.
Model GFLOPS/FPS #params AP AP 50 AP75 APS APM APL
...
4.1 Comparison with F aster R-CNN
...
behind in AP S too. Faster R-CNN and DETR with ResNet-101 backbone show
...
stage of the decoding (Fig. 4). Both AP and AP 50 improve after every layer,
...
AP50 No NMS
AP50 NMS=0.7
Fig. 4: AP and AP 50 performance after each de-
...
Table 4: Effect of loss components on AP. We train two models turning off l1 loss, and
...
class l1 GIoU AP ∆ AP50 ∆ APS APM APL
...
set. We create a synthetic image 3 to verify the generalization ability of DETR
...
Multi head attentionInput image
(3 x H x W)
Box embeddings
(d x N)
Encoded image
...
for each object and computes multi-head (withM heads) attention scores of this
...
different masks.
T raining details. We train DETR, DETR-DC5 and DETR-R101 models fol-
...
the break-down on things (PQ th) and stuff (PQ st). We also report the mask
...
obtains competitive PQ th. We also evaluated our method on the test set of the
...
13. He, K., Girshick, R., Doll ́ ar, P.: Rethinking imagenet pre-training. In: ICCV (2019)
14. He, K., Gkioxari, G., Doll ́ ar, P., Girshick, R.B.: Mask R-CNN. In: ICCV (2017)
...
In: CVPR (2018)
18. Kirillov, A., Girshick, R., He, K., Doll ́ ar, P.: Panoptic feature pyramid networks.
In: CVPR (2019)
...
22. Lin, T.Y., Doll ́ ar, P., Girshick, R., He, K., Hariharan, B., Belongie, S.: Feature
pyramid networks for object detection. In: CVPR (2017)
23. Lin, T.Y., Goyal, P., Girshick, R.B., He, K., Doll ́ ar, P.: Focal loss for dense object
detection. In: ICCV (2017)
24. Lin, T.Y., Maire, M., Belongie, S., Hays, J., Perona, P., Ramanan, D., Doll ́ ar, P.,
...
27. L ̈ uscher, C., Beck, E., Irie, K., Kitza, M., Michel, W., Zeyer, A., Schl ̈ uter, R., Ney,
...
39. Rezatofighi, S.H., Kaskman, R., Motlagh, F.T., Shi, Q., Cremers, D., Leal-Taix ́ e,
...
(2015)
42. Salvador, A., Bellver, M., Baradad, M., Marqu ́ es, F., Torres, J., Gir ́ o, X.: Recurrent
...
(2019)
53. Zhou, X., Wang, D., Kr ̈ ahenb ̈ uhl, P.: Objects as points. arXiv:1904.07850 (2019)
...
Multi-head The general form of multi-head attention withM heads of dimen-
...
lengthNkv (with the same number of channels d for simplicity of exposition), T
is the weight tensor to compute the so-called query, key and value embeddings,
andL is a projection matrix. The output is the same size as the query sequence.
To fix the vocabulary before giving details, multi-headself-attention (mh-s-attn)
...
heads followed by a projection withL. The common practice [47] is to use residual
connections, dropout and layer normalization. In other words, denoting ̃Xq =
mh-attn(Xq,X kv,T,L ) and ̄ ̄X (q) the concatenation of attention heads, we have
X′
q = [attn(Xq,X kv,T 1);...; attn(Xq,X kv,TM)] (5)
 ̃Xq = layernorm
(
Xq + dropout(LX′
q)
...
attn(Xq,X kv,T′), depends on additional positional encoding Pq∈ Rd×Nq and
...
is the aggregation of values weighted by attention weights: Thei-th row is given
by attni(Xq,X kv,T′) =∑Nkv
j=1αi,jVj.
F eed-forward network (FFN) layers The original transformer alternates
...
in our loss, together with a l1 loss on ˆb:
Lbox(bσ(i),ˆbi) =λiouLiou(bσ(i),ˆbi) +λL1||bσ(i)− ˆbi||1, (9)
whereλiou,λ L1∈ R are hyperparameters andLiou(·) is the generalized IoU [38]:
Liou(bσ(i),ˆbi) = 1−
(|bσ(i)∩ ˆbi|
|bσ(i)∪ ˆbi|
−|B(bσ(i),ˆbi)\bσ(i)∪ ˆbi|
...
σ( ˆm) +m + 1 (11)
whereσ is the sigmoid function. This loss is normalized by the number of objects.
...
ityO(d2HW +d(HW )2):O(d′d) is the cost of computing a single query/key/value
embeddings (and Md′ =d), whileO(d′(HW )2) is the cost of computing the at-
tention weights for one head. Other computations are negligible. In the decoder,
each self-attention is inO(d2N +dN 2), and cross-attention between encoder and
...
first few epochs.
T ransformer We train the transformer with a learning rate of 10 −4. Additive
...
Losses We use linear combination of l1 and GIoU losses for bounding box re-
gression withλL1 = 5 andλiou = 2 weights respectively. All models were trained
