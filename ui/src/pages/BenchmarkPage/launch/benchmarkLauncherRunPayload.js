...
import { isValidHttpUrl, normalizeFamily, normalizeString } from "./benchmarkLauncherInternals.js";
...
export function humanizeLauncherError(errorLike) {
  ...
  if (detail.startsWith("model_profile_not_supported_for_family:")) {
    const [, profileId, fam] = detail.split(":");
    return `Model profile ${profileId || "unknown"} is not supported for ${fam || "this family"}.`;
  }
  if (detail.startsWith("gold_source_not_supported_for_family:")) {
    const [, goldSource, fam] = detail.split(":");
    return `Gold source ${goldSource || "unknown"} is not supported for ${fam || "this family"}.`;
  }
