...
import GraphCanvasLabelsControl from "./GraphCanvasLabelsControl.jsx";
 ...
 *   onApplyViewPreset?: (preset: "overview" | "reading") => void,
 * }} props
 */
export default function GraphCanvasViewToolbar({
  ...
  onUnpinAll,
  unpinDisabled,
  /** When true, Fit / Reset / Center live on the canvas overlay (see GraphCanvasMvp). */
  hideViewControls = false,
  onApplyViewPreset,
}) {
  const tk = useTheme().appTokens;
  const presetToggleValue =
    canvasLabelMode === "interaction" && hoverNeighborsLabels
      ? "overview"
      : canvasLabelMode === "adaptive" && hoverNeighborsLabels
        ? "reading"
        : null;
  const sectionSx = {
    fontSize: "0.62rem",
  ...
  return (
    <Box sx={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: 0.65, px: 1, py: 0.5, borderBottom: `1px solid ${tk.border.default}` }}>
        ...
        <>
          ...
          <Box sx={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: 0.35 }}>
            <Typography component="span" sx={sectionSx}>
              {t("graph.canvas.toolbarSectionPreset")}
            </Typography>
            <ToggleButtonGroup
              size="small"
              exclusive
              ...
              onChange={(_, v) => {
                if (v === "overview" || v === "reading") onApplyViewPreset(v);
              }}
              aria-label={t("graph.canvas.presetAria")}
              sx={toggleSx}
            >
              <ToggleButton value="overview" title={t("graph.canvas.presetOverviewTooltip")}>
                {t("graph.canvas.presetOverview")}
              </ToggleButton>
              <ToggleButton value="reading" title={t("graph.canvas.presetReadingTooltip")}>
                {t("graph.canvas.presetReading")}
              </ToggleButton>
            </ToggleButtonGroup>
          </Box>
        </>
      ...
      >
        ...
        {graphColorBy === "community" ? (
          <Typography sx={{ fontSize: "0.62rem", color: tk.text.faint, maxWidth: 120, lineHeight: 1.25, ml: 0.25 }}>
      ...
      <Box sx={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: 0.35 }}>
        ...
        <GraphCanvasLabelsControl
          t={t}
          canvasLabelMode={canvasLabelMode}
          onCanvasLabelModeChange={onCanvasLabelModeChange}
          hoverNeighborsLabels={hoverNeighborsLabels}
          onHoverNeighborsLabelsChange={onHoverNeighborsLabelsChange}
        />
      </Box>
      <Divider orientation="vertical" flexItem sx={{ borderColor: tk.border.default, alignSelf: "stretch", minHeight: 28 }} />
          ...
          <Tooltip title={t("graph.canvas.repulsionTooltip")}>
            <Box sx={{ width: 128, px: 0.25, cursor: "help" }}>
              <Box sx={{ display: "flex", alignItems: "baseline", gap: 0.5, mb: 0.15 }}>
                <Typography sx={{ fontSize: "0.62rem", color: tk.text.faint, fontFamily: "monospace", letterSpacing: "0.02em" }}>
                  sim
                </Typography>
