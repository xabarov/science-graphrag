...
function ChatDetailLevelToggle({ t, tk, detailLevel, onToggleDetailLevel, iconButtonSx = {} }) {
  ...
  const showDiscoveryTooltip = detailLevel === "simple" && !readChatDetailDiscoveryHintDismissed();
      ...
      title={
        showDiscoveryTooltip
          ...
          : detailLevel === "detailed"
            ? t("chat.chrome.detailModeDetailed")
            : t("chat.chrome.detailModeSimple")
      }
      sx={{
        flexShrink: 0,
        color: tk.text.muted,
  ...
  if (!showDiscoveryTooltip) {
    return iconButton;
  }

  return (
    <Tooltip
      title={t("chat.chrome.detailDiscoveryHint")}
      open
      ...
      slotProps={{
        tooltip: {
          sx: { maxWidth: 300, fontSize: "0.75rem", lineHeight: 1.45 },
        },
      }}
    >
      {iconButton}
