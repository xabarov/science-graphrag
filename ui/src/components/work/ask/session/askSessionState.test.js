...
describe("storage guards", () => {
  ...
  it("returns safe defaults when localStorage is unavailable", () => {
    expect(createAskSession("scope-guard", "Title")).toBe("");
    expect(removeAskSession("scope-guard", "session-id")).toEqual({
      nextActiveId: null,
      createdFallback: false,
    });
    expect(getActiveSessionEntries("scope-guard")).toEqual([]);
  });
});
