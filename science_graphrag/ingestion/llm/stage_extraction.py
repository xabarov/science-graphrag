...
SYSTEM_FENCE = (
    "You extract structured metadata from scholarly text in Markdown. "
    "Return only fields you are confident about; use null for unknown. "
    "Do not invent DOIs or titles."
)
...
def extract_stages_llm_first(
    ...
    *,
    ...
    user_meta = (
        "From the following scholarly markdown, extract work-level metadata.\n\n"
        f"---\n{meta_text}"
    )
    ...
    with llm_span(
        ...
        parsed, err = extractor.extract_maybe(
            WorkMetadataLLM,
            system=SYSTEM_FENCE + " Focus on title, abstract, venue, year, DOI, arXiv, language.",
            user=user_meta,
        )
    if err:
        diag.fallback_reasons.append(
    ...
    user_auth = (
        "From the following scholarly markdown, extract authors in order. "
        "For each author provide full name, affiliations (strings), "
        "corresponding flag and email if present.\n\n"
        f"---\n{meta_text}"
    )
    ...
    with llm_span(
        ...
        aparsed, aerr = extractor.extract_maybe(
            AuthorshipsLLM,
            system=SYSTEM_FENCE + " Authors only; preserve order.",
            user=user_auth,
        )
    if aerr:
        diag.fallback_reasons.append(
    ...
    user_refs = (
        "Extract bibliography entries from the text. For each entry include "
        "raw_reference (full line or paragraph), "
        "doi if present, title if obvious, publication year if present.\n\n---\n"
        f"{ref_text}"
    )
    ...
    with llm_span(
        ...
        rparsed, rerr = extractor_refs.extract_maybe(
            ...
            system=(
                SYSTEM_FENCE
                + " References list only; one item per numbered or bulleted entry when possible."
            ),
            user=user_refs,
        )
    if rerr:
