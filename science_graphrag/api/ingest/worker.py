...
from .dto import job_record_to_view, now_iso
...
def _sse_progress_canonical_fields(job_id: str, settings: Settings) -> dict[str, Any]:
    """Re-read job from DB and attach canonical progress fields for ``stage_progress`` SSE."""
    rec = _registry(settings).get_job(job_id)
    if rec is None:
        return {}
    try:
    ...
    # Always set so SSE clients can clear a prior True without relying on omitted keys.
...
def _stage_event_publisher(
    ...
    def _progress(stage_name: IngestStage, metrics: dict[str, Any]) -> None:
        ...
        try:
            extras = _sse_progress_canonical_fields(job_id, settings)
            if extras:
